...
router.patch("/social/me", async (req, res): Promise<void> => {
  ...
  if (!user) { res.status(404).json({ error: "User not found" }); return; }
  const VALID_FRAMES = ["none","golden_crown","sakura","ice_crystal","fire_blaze","neon_green","purple_galaxy","rainbow","rose_garden","silver_steel","midnight_blue","emerald","blood_red","pastel_dream","sunset","diamond","toxic","obsidian","crystal","sunset_glow","toxic_green"];
