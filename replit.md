...
- `DashboardLayout.tsx` — Sidebar with owner groups list, user profile (with equipped avatar frame), license info, Tools section (AI Assistant, Competitors, Limited Sniper), Community & Settings links (Community shows unread DM badge), Online Users presence section (green dot indicators, avatars, real-time via WebSocket). Wrapped with `PresenceProvider` context for app-wide presence sharing. Unread DM count polled every 15s (skipped on /community). Avatar frame fetched from `/api/social/me` and synced via `avatar-frame-changed` custom event.
- **Presence System** — `PresenceContext.tsx` wraps DashboardLayout; `usePresence.ts` hook manages WS connection, online users, presenceMap, fetchPresenceFor (batch REST), isOnline/getLastSeen helpers. `POST /api/presence/status` returns online/lastSeen for user IDs. `signaling.ts` caches lastSeen in-memory + persists to `platformUsers.lastSeen` DB column on disconnect. Community.tsx shows green dot indicators on DM avatars, "online" / "last seen X ago" text in DM list and chat header.
...
- `Community.tsx` — Social hub with VK-style left sidebar nav: Feed (posts/likes/comments), Forum (Suggestions/Off-topic/Q&A with voting & replies), Discover (find developers), Friends (requests, chat), Chat (DMs + group chats + 1-on-1 voice calls + group voice calls + voice messages + media attachments), Marketplace, Accessories (profile decorations via mini-games). Media attachments: images/files can be sent in DMs and group chats via paperclip button; attachments stored as base64 data URLs in `imageUrl` column (format: `[attachments:JSON]`); server validates format prefix and 10MB max size. Group voice calls use mesh WebRTC topology (each participant has one RTCPeerConnection per remote peer). Signaling: `useGroupCall.ts` hook + server `signaling.ts` group-call-* message types + `GroupCallOverlay.tsx` UI with expandable participant grid, mute/deafen/leave controls. Group call ring notifications: server queries `groupChatMembers` (joined with `platformUsers` to map platform→roblox IDs) and sends `group-call-ring` to all online members not in the call; client shows incoming call popup with Join/Dismiss buttons, auto-dismisses after 30s. Screen sharing in group calls: `toggleScreenShare` uses `getDisplayMedia`, adds video track to all peer connections via renegotiation (existing PCs are reused, not recreated); `group-track-state` WS message notifies peers of screen share start/stop; `GroupCallOverlay` shows screen share video in expanded view and monitor icon on sharing peer's avatar.
- `AccessoriesTab.tsx` — Profile accessories system with 5 sub-tabs: **Quests** (default tab — earn accessories by completing objectives: send messages, add friends, create posts, get likes, win minigames, win duels; rarity determines difficulty; start quest → track live progress → claim reward), **Duels** (competitive PvP games — Rock Paper Scissors, Number War, Dice Battle, Roblox Trivia, Coin Battle; create open challenges, accept opponent duels, view results), **Inventory** (equip/unequip items per category), **Catalog** (browse all accessories by category/rarity), **Leaderboard** (top players by wins, min 3 games). DB tables: `accessories`, `user_accessories`, `minigame_plays`, `game_challenges`, `quests`, `user_quests`. 37 seeded accessories across 5 categories (frame, badge, background, title, effect) with 4 rarities. 37 quests (one per accessory) with 6 quest types. Duel challenges expire after 24h, max 5 active per user.
- `Profile.tsx` — User profile page with hero card, Roblox stats, bio. Avatar frame customization via Palette button → modal dialog with 17 gradient frames (4 rarities: common/rare/epic/legendary). Frames stored in `platform_users.avatar_frame` column. `AvatarWithFrame` component renders CSS gradient borders + glow shadows around avatar. Changes sync to sidebar via `avatar-frame-changed` custom event.
