...
- `AccessoriesTab.tsx` — Profile accessories system with 4 sub-tabs: **Duels** (competitive PvP games — Rock Paper Scissors, Number War, Dice Battle, Roblox Trivia, Coin Battle; create open challenges, accept opponent duels, view results), **Inventory** (equip/unequip items per category), **Catalog** (browse all accessories by category/rarity), **Leaderboard** (top players by wins, min 3 games). Winners receive random accessories with weighted rarity. DB tables: `accessories`, `user_accessories`, `minigame_plays`, `game_challenges`. 27 seeded accessories across 5 categories (frame, badge, background, title, effect) with 4 rarities. Duel challenges expire after 24h, max 5 active per user.
