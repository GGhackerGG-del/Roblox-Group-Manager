...
- `Community.tsx` — Social hub with VK-style left sidebar nav: Feed (posts/likes/comments), Forum (Suggestions/Off-topic/Q&A with voting & replies), Discover (find developers), Friends (requests, chat), Chat (DMs + group chats + voice calls + voice messages + media attachments), Marketplace, Accessories (profile decorations via mini-games). Media attachments: images/files can be sent in DMs and group chats via paperclip button; attachments stored as base64 data URLs in `imageUrl` column (format: `[attachments:JSON]`); server validates format prefix and 10MB max size.
