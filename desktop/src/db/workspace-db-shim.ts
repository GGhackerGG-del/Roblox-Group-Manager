...
const getDbInstance = () => {
  const db = (globalThis as any).__limitedInkDb;
...
export const pool = {
  query: () => { throw new Error("PostgreSQL pool not available in desktop mode"); },
  connect: () => { throw new Error("PostgreSQL pool not available in desktop mode"); },
  end: () => Promise.resolve(),
};
